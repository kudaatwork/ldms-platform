...
/** Trimmed API gateway base (no trailing slash). Must not be the UI dev-server origin. */
export function apiBaseUrl(): string {
  return (environment.gatewayUrl ?? environment.apiUrl).replace(/\/$/, '');
}

/** Absolute URL for an LDMS path segment (must start with `/ldms-`). */
 ...
 * e.g. `ldmsServiceUrl('organization-management', 'organization', 'find-by-multiple-filters', 'frontend')`.
 */
...
/** True when the request targets the API gateway (absolute URL or same-origin {@code /ldms-*} / {@code /api}). */
