...
export class VerificationService {
  ...
  requestPhoneVerification(): Observable<void> {
    return this.http.post<unknown>(`${this.userBase}/request-phone-verification`, {}).pipe(
      map((resp) => {
  ...
  confirmPhoneVerification(otp: string): Observable<void> {
    return this.http
      .post<unknown>(`${this.userBase}/confirm-phone-verification`, { otp: otp.trim() })
      .pipe(
        map((resp) => {
          this.ensureSuccess(resp);
        }),
