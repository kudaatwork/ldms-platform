...
export class LoginAnalyticsPageComponent implements OnInit, OnDestroy {
  ...
  /** Shown when audit API returns 403 (missing SEARCH_AUDIT_LOGS in JWT). */
  permissionError = '';
  /** Organisation workspace label for scoped audit views. */
  ...
  constructor(
    ...
    private readonly auditLogPortal: AuditLogPortalService,
  ...
  private runLoginQuery(): Observable<unknown> {
    ...
    return this.auditLogPortal.queryLoginEventsPage(payload).pipe(
      ...
      catchError((error: unknown) => {
        ...
        this.applyLoginRows([], 0);
        const msg = this.errorMessage(error, 'Failed to load login events.');
        this.permissionError = msg.toLowerCase().includes('permission') ? msg : this.permissionError;
  ...
  private runActivityQuery(): Observable<unknown> {
    ...
    return this.auditLogPortal.queryUserActivityPage(payload, username).pipe(
      tap(({ rows, totalElements }) => {
