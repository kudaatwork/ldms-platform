...
@Component({
  selector: 'app-org-managers-page',
  templateUrl: './org-managers-page.component.html',
  styleUrl: './org-managers-page.component.scss',
  standalone: false,
})
export class OrgManagersPageComponent implements OnInit, OnDestroy {
  loading = true;
  actionInProgress = false;
  exporting = false;
  error = '';
  managers: UserListRow[] = [];
  ...
  get filteredManagers(): UserListRow[] {
    const q = this.searchQuery.trim().toLowerCase();
    return this.managers.filter((manager) => {
      if (this.columnFilters.status === true && manager.status !== 'ACTIVE') {
        return false;
      }
      if (this.columnFilters.status === false && manager.status === 'ACTIVE') {
        return false;
      }
      const nameNeedle = this.columnFilters.name.trim().toLowerCase();
      if (nameNeedle && !manager.name.toLowerCase().includes(nameNeedle)) {
        return false;
      }
      if (!q) {
        return true;
      }
      const hay = [manager.name, manager.email, manager.phoneNumber, manager.username].join(' ').toLowerCase();
  ...
  ngOnInit(): void {
    this.title.setTitle('Managers | Organization management');
    this.loadBranches();
    this.reload();
  }
  ...
  reload(): void {
    ...
    this.usersPortal
      .queryUsers({
        ...
        size: 1000,
        searchQuery: '',
        columnFilters: { userTypeName: 'Branch Manager' },
        organizationId,
      })
      .pipe(
      ...
      .subscribe({
        ...
        error: (err: Error) => {
          this.error = err.message ?? 'Could not load managers.';
        },
      });
  }
    ...
    this.usersPortal
      ...
      .subscribe({
        next: (blob) => {
          ...
          a.href = url;
          a.download = `managers-export.${format === 'xlsx' ? 'xlsx' : format === 'pdf' ? 'pdf' : 'csv'}`;
          a.click();
  ...
  openCreate(): void {
    ...
    if (!this.branches.length) {
      this.snackBar.open('Create a branch first — managers belong to a branch.', 'Close', { duration: 5000 });
      return;
    }
    this.dialog
      .open(BranchStaffDialogComponent, {
        data: { role: 'manager', organizationId, branches: this.branches },
        width: '640px',
        maxWidth: '96vw',
  ...
  openEdit(manager: UserListRow): void {
    const organizationId = this.orgContext.organizationId;
    if (!organizationId) {
      return;
    }
    this.dialog
      .open(BranchStaffDialogComponent, {
        data: { role: 'manager', organizationId, branches: this.branches, user: manager },
        width: '640px',
        maxWidth: '96vw',
        panelClass: 'lx-dialog-panel',
      })
  ...
  deleteManager(manager: UserListRow): void {
    this.dialog
      .open(DeleteConfirmDialogComponent, {
        data: {
          title: 'Remove manager?',
          message: `This will remove ${manager.name} from the organisation. This action cannot be undone.`,
          confirmLabel: 'Remove',
        },
        width: '420px',
        maxWidth: '96vw',
      ...
      .subscribe((confirmed: boolean) => {
        if (confirmed) {
          ...
          this.usersPortal
            .deleteUser(manager.id)
            .pipe(finalize(() => (this.actionInProgress = false)))
            .subscribe({
              next: () => {
                this.snackBar.open('Manager removed', 'Close', { duration: 3000, panelClass: ['app-snackbar-success'] });
                this.reload();
              },
  ...
  managerBranchName(manager: UserListRow): string {
    if (manager.branchId == null) {
      return '—';
    }
    const branch = this.branches.find((b) => b.id === manager.branchId);
