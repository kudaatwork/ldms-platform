      ...
      .subscribe({
        next: (resp) => {
          if (this.usersPortal.isUserMutationFailure(resp)) {
            this.error = this.usersPortal.formatUserMutationError(resp, 'Update failed. Check phone number format.');
            return;
          }
        ...
        error: (err: unknown) => {
          if (this.usersPortal.isUserMutationFailure(err)) {
            this.error = this.usersPortal.formatUserMutationError(err, 'Update failed. Check phone number format.');
            return;
          }
