...
export class UserEditSecurityDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.ensureCreateDefaults();
    if (this.selfService) {
      this.refreshFromServer(this.usersPortal.getMyUserSecurity());
      return;
    }
    if (Number.isFinite(this.securityId) && this.securityId > 0) {
      this.refreshFromServer(this.usersPortal.getUserSecurityById(this.securityId));
      return;
    }
    this.error =
  ...
  private refreshFromServer(source: ReturnType<UsersPortalService['getMyUserSecurity']>): void {
    this.loading = true;
    this.cdr.markForCheck();
    source
      .pipe(
      ...
      .subscribe((row) => {
        ...
        if (!this.securityQuestion_1.trim() && !this.twoFactorAuthSecret.trim()) {
          this.error =
            'No security record is linked yet. Fill in the fields below and Save to create one.';
        } else if (!this.securityAnswer_1.trim()) {
          this.error =
            'Re-enter your security answers below (they are not shown after save). Other fields are pre-filled where available.';
        }
        this.cdr.markForCheck();
      });
  }
  ...
  generateTwoFactorSecret(silent = false): void {
  ...
  save(): void {
    ...
    if (
      !this.selfService &&
      (!Number.isFinite(this.initialUserId) || this.initialUserId <= 0)
    ) {
    ...
    const save$ = this.selfService
      ? this.usersPortal.saveMyUserSecurity({
      ...
      : Number.isFinite(this.securityId) && this.securityId > 0
        ? this.usersPortal.updateUserSecurity({ id: this.securityId, ...basePayload })
        : this.usersPortal.createUserSecurity(basePayload);
    const okMessage =
      Number.isFinite(this.securityId) && this.securityId > 0
