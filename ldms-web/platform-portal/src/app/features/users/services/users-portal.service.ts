...
export class UsersPortalService {
  private static readonly EXPORT_PAGE_SIZE = 10_000;

  /** Platform portal: {@code frontend} with organisation-scoped filters. */
  private readonly base = ldmsApiUrl('/ldms-user-management/v1/frontend');
  ...
  exportUsers(q: UsersQuery, format: LxExportFormat): Observable<Blob> {
    const body = this.buildUsersFilterBody({
      ...q,
      page: 0,
      size: UsersPortalService.EXPORT_PAGE_SIZE,
    });
  ...
  /**
   * Distinct login usernames for an organisation workspace — lightweight alternative to
   * {@link queryUsersForOrganization} for audit log scoping.
   */
  queryOrganizationUsernames(organizationId: number): Observable<string[]> {
    if (!Number.isFinite(organizationId) || organizationId < 1) {
      return of([]);
    }
    return this.http
      .get<unknown>(`${this.base}/user/usernames-by-organization-id/${organizationId}`)
      .pipe(
        map((resp) => this.extractOrganizationUsernames(resp)),
        catchError((err) => {
  ...
  queryUsers(q: UsersQuery): Observable<{ rows: UserListRow[]; totalElements: number }> {
    const orgId = q.organizationId ?? this.orgContext.organizationId;
    const useOrgListing =
      orgId != null &&
      Number.isFinite(orgId) &&
      orgId > 0 &&
      (q.branchId == null || q.branchId <= 0) &&
      (q.userGroupId == null || q.userGroupId <= 0);
    if (useOrgListing) {
      return this.queryUsersForOrganization(orgId).pipe(
        switchMap(({ rows }) => {
          const paged = this.paginateFilteredUserRows(rows, q);
          if (paged.rows.length > 0 || paged.totalElements > 0) {
            return of(paged);
          }
          return this.queryUsersByMultipleFilters(q).pipe(
            map((fallback) => this.paginateFilteredUserRows(fallback.rows, q)),
          );
        }),
      );
    }
    return this.queryUsersByMultipleFilters(q);
  }

  private queryUsersByMultipleFilters(
    q: UsersQuery,
  ): Observable<{ rows: UserListRow[]; totalElements: number }> {
  ...
  queryUserGroups(q: UsersQuery): Observable<{ rows: Record<string, unknown>[]; totalElements: number }> {
    ...
    return this.http
      ...
      .pipe(
      map((resp) => {
        const page = this.extractPagedResultWithAlternateKeys(resp, 'userGroupDtoPage', [
          'userGroupPage',
          'user_group_dto_page',
          'page',
        ]);
        const sorted = this.sortUserGroupsAdministratorFirst(page.rows);
        return { rows: sorted, totalElements: page.totalElements };
      }),
      catchError((err) => this.emptyPageOnNotFound(err)),
    );
  }
  ...
  getUserProfileBundle(userId: number): Observable<UserProfileBundle> {
    return this.http.get<unknown>(`${this.base}/user/find-by-id/${userId}`).pipe(
      switchMap((resp) => {
        ...
        return this.resolveUserProfileBundleFromOrganization(userId);
      }),
      catchError((err) => {
        if (err instanceof HttpErrorResponse && [401, 403, 404].includes(err.status)) {
          return this.resolveUserProfileBundleFromOrganization(userId).pipe(
            switchMap((fallback) =>
              fallback.user
                ? of(fallback)
                : this.resolveSelfProfileBundleWhenIdsMatch(userId),
            ),
          );
        }
        return throwError(() => err);
  ...
  /** Draft for the address editor when no row exists yet. */
  addressDraftForEdit(
    user: Record<string, unknown> | null,
    address: Record<string, unknown> | null,
  ...
  /** Links a new address to the user via profile update (backend creates address when missing). */
  ...
  private extractUserDtoFromFindByIdResponse(response: unknown): Record<string, unknown> | null {
    ...
    const list = this.extractUserDtoList(response);
    if (list.length === 1) {
      return list[0];
    }
  ...
  /** Signed-in user's security row ({@code GET /user-security/me}). */
  getMyUserSecurity(): Observable<Record<string, unknown> | null> {
    return this.http.get<unknown>(`${this.base}/user-security/me`).pipe(
      map((resp) => {
        const obj = this.toObj(this.parsePossiblyStringifiedJson(resp));
        if (obj && obj['success'] === false) {
          return null;
        }
        return this.extractSingleDto(resp, 'userSecurityDto');
      }),
      catchError(() => of(null)),
    );
  }

  /** Creates or updates security for the signed-in user ({@code PUT /user-security/me}). */
  ...
  queryOrganizationsForSelect(): Observable<IdLabelOption[]> {
    ...
    if (sessionOrgId) {
      const myUrl = ldmsServiceUrl('organization-management', 'organization', 'my', 'frontend');
      return this.http.get<unknown>(myUrl).pipe(
        map((resp) => {
          const org = this.extractSingleDto(resp, 'organizationDto');
          const id = Number(org?.['id'] ?? sessionOrgId);
          const name = String(org?.['name'] ?? '').trim();
          return Number.isFinite(id) && id > 0 && name ? [{ id, label: name }] : [];
        }),
        catchError(() => of([])),
      );
    }
    const url = ldmsServiceUrl('organization-management', 'organization', 'find-by-multiple-filters', 'frontend');
    return this.http
      .post<unknown>(url, {
        page: 0,
        size: 500,
  ...
  queryBranchesForOrganization(organizationId: number): Observable<IdLabelOption[]> {
    ...
    const sessionOrgId = this.orgContext.organizationId;
    const orgBase = ldmsServiceUrl('organization-management', 'organization');
    const url =
      sessionOrgId === organizationId
        ? `${orgBase}/my`
        : ldmsServiceUrl('organization-management', 'organization', String(organizationId), 'frontend');
    return this.http.get<unknown>(url).pipe(
      map((resp) => this.mapOrganizationBranchOptions(resp)),
      catchError(() => of([])),
    );
  }

  private mapOrganizationBranchOptions(resp: unknown): IdLabelOption[] {
    const org = this.extractSingleDto(resp, 'organizationDto');
  ...
  private extractOrganizationUsernames(response: unknown): string[] {
    const obj = this.toObj(this.parsePossiblyStringifiedJson(response));
    if (!obj || obj['success'] === false) {
      return [];
    }
    const direct = obj['usernames'];
    if (Array.isArray(direct)) {
      return [
        ...new Set(
          direct.map((u) => String(u ?? '').trim()).filter((u): u is string => !!u),
        ),
      ];
    }
    return [
      ...new Set(
        this.extractUserDtoList(response)
  ...
  private extractUserDtoList(response: unknown): Record<string, unknown>[] {
    ...
    for (const src of candidates) {
      for (const key of ['userDtoList', 'users', 'userDtos'] as const) {
        const list = src[key];
        if (Array.isArray(list)) {
          return list.filter(
            (r): r is Record<string, unknown> => r !== null && typeof r === 'object' && !Array.isArray(r),
      ...
      const singleton = this.asRecord(src['userDto']);
      if (singleton && singleton['id'] != null) {
        return [singleton];
      }
    }
  ...
  private paginateFilteredUserRows(
    rows: UserListRow[],
    q: UsersQuery,
  ): { rows: UserListRow[]; totalElements: number } {
    const filtered = this.filterUserListRows(rows, q);
    const totalElements = filtered.length;
    const start = Math.max(0, q.page) * Math.max(1, q.size);
    return {
      rows: filtered.slice(start, start + Math.max(1, q.size)),
      totalElements,
    };
  }

  private filterUserListRows(rows: UserListRow[], q: UsersQuery): UserListRow[] {
    const qLower = q.searchQuery.trim().toLowerCase();
    ...
    return rows.filter((row) => {
      ...
      if (emailF && !row.email.toLowerCase().includes(emailF)) {
        return false;
      }
      const userF = (cf['username'] ?? '').trim().toLowerCase();
      if (userF && !row.username.toLowerCase().includes(userF)) {
        return false;
      }
      const phoneF = (cf['phoneNumber'] ?? '').trim().toLowerCase();
      if (phoneF && !row.phoneNumber.toLowerCase().includes(phoneF)) {
        return false;
      }
      const statusF = (cf['statusLabel'] ?? '').trim().toLowerCase();
      if (statusF && !row.statusLabel.toLowerCase().includes(statusF)) {
        return false;
      }
      if (!qLower) {
        return true;
      ...
      return (
        ...
        row.username.toLowerCase().includes(qLower) ||
        row.email.toLowerCase().includes(qLower) ||
        row.role.toLowerCase().includes(qLower)
      );
    });
  }

  private sortUserGroupsAdministratorFirst(rows: Record<string, unknown>[]): Record<string, unknown>[] {
    return [...rows].sort((a, b) => {
      const nameA = String(a['name'] ?? '').trim();
      const nameB = String(b['name'] ?? '').trim();
      const adminA = nameA.toLowerCase() === 'administrator';
      const adminB = nameB.toLowerCase() === 'administrator';
      if (adminA && !adminB) {
        return -1;
      }
      if (!adminA && adminB) {
  ...
  /**
   * When {@code find-by-id} is denied or missing, reuse the org-scoped list API (same as the users table).
   */
  /** When {@code find-by-id} is denied, load the signed-in row via {@code GET /user/me}. */
  ...
  private resolveUserProfileBundleFromOrganization(userId: number): Observable<UserProfileBundle> {
    const orgId = this.orgContext.organizationId;
    if (!Number.isFinite(userId) || userId <= 0 || !orgId || orgId < 1) {
      return of(this.emptyProfileBundle());
    }
    return this.http.get<unknown>(`${this.base}/user/find-by-organization-id/${orgId}`).pipe(
      map((resp) => {
        const match =
          this.extractUserDtoList(resp).find((row) => Number(row['id'] ?? 0) === userId) ?? null;
        if (!match) {
          return this.emptyProfileBundle();
        }
        return {
          user: match,
          account: this.extractNestedDto(match, ['userAccountDto', 'userAccount']),
          security: this.extractNestedDto(match, ['userSecurityDto', 'userSecurity']),
          address: this.extractNestedDto(match, ['addressDto', 'address']),
          password: this.extractNestedDto(match, ['userPasswordDto', 'userPassword']),
        };
      }),
      catchError(() => of(this.emptyProfileBundle())),
    );
