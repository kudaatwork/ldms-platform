...
import {
  ...
  type BranchFormDialogData,
  type BranchFormDialogResult,
...
export class BranchesListComponent implements OnInit, OnDestroy {
  readonly pageLead =
    'Physical and operational branches registered under organisations — head offices, regional sites, and contact points.';
  ...
  readonly sampleCsvDescription =
    'Branch imports use ORGANIZATION_ID, branch name, code, region, and contact columns. Leave ID empty when creating new rows.';
  ...
  displayedColumns = ['branchName', 'organization', 'region', 'contact', 'flags', 'status', 'actions'];
  dataSource = new MatTableDataSource<BranchListRow>([]);
  totalRecords = 0;
  pageIndex = 0;
  ...
  viewRow(row: BranchListRow): void {
    this.openFormDialog({ action: 'view', row });
  }

  confirmDelete(row: BranchListRow): void {
    this.dialog
      .open(DeleteConfirmDialogComponent, {
        width: '420px',
        maxWidth: '92vw',
        data: { entityLabel: `branch "${row.branchName}"` },
      })
      .afterClosed()
      ...
      .subscribe((confirmed: boolean) => {
        ...
        this.orgService
          ...
          .subscribe({
            next: () => {
              this.showSuccess('Branch deleted.');
  ...
  downloadSampleCsv(): void {
    const template = this.orgService.getBranchSampleCsv();
  ...
  exportAs(format: 'csv' | 'xlsx' | 'pdf'): void {
    ...
    this.orgService
      ...
      .subscribe({
        next: (blob) => {
          downloadBlob(blob, exportFilename('branches', format));
          this.showSuccess(`Exported branches as ${format.toUpperCase()}.`);
        },
        error: (err: Error) => {
  ...
  private runTableQuery(opts?: {
    background?: boolean;
  }): Observable<{ rows: BranchListRow[]; totalElements: number }> {
    ...
    return this.orgService
      ...
      .pipe(
        ...
        catchError((err: Error) => {
          ...
          this.loadError = err.message ?? 'Failed to load branches';
