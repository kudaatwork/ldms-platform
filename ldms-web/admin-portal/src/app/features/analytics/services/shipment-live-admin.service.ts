...
import type { ShipmentLiveView, TripLiveSnapshot, TripRouteWaypoint } from '../models/shipment-live.model';

@Injectable({ providedIn: 'root' })
export class ShipmentLiveAdminService {
  private readonly tripLiveBase = ldmsServiceUrl('trip-tracking', 'trip-live', undefined, 'backoffice');
  ...
  fetchShipmentLive(organizationId: number, shipmentId: number): Observable<ShipmentLiveView | null> {
    if (!shipmentId || shipmentId < 1) {
      return of(null);
    }

    return this.http
      .get<unknown>(`${this.tripLiveBase}/by-shipment/${shipmentId}`)
      .pipe(
        map((resp) => this.mapTripSnapshot(this.extractSingle(resp))),
        switchMap((snapshot) =>
          this.resolveOrganizationName(organizationId).pipe(
  ...
  private resolveOrganizationName(organizationId: number): Observable<string> {
    if (!organizationId || organizationId < 1) {
      return of('—');
    }
    return this.organizations.getOrganizationProfile(organizationId).pipe(
      map((org) => org.name?.trim() || `Organisation #${organizationId}`),
      catchError(() => of(`Organisation #${organizationId}`)),
    );
  }
