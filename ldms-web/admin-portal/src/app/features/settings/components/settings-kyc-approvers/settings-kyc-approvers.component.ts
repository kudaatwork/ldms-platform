...
import { SettingsAdminService, type KycApprovalPolicy } from '../../services/settings-admin.service';
...
export class SettingsKycApproversComponent implements OnInit, OnDestroy {
    ...
    forkJoin({
      ...
      .subscribe({
        next: ({ policy, orgs, groups }) => {
          ...
          this.policy = policy;
          this.draftDefaultStages = policy.defaultRequiredApprovalStages;
          this.orgRows = orgs.rows.map((row) => ({
            id: row.id,
            name: row.applicant || row.name,
            classificationLabel: row.classificationLabel,
            overrideStages: row.kycRequiredApprovalStages ?? null,
            effectiveStages: row.effectiveKycRequiredApprovalStages ?? row.kycRequiredApprovalStages ?? policy.defaultRequiredApprovalStages,
            saving: false,
          }));
  ...
  private reloadOrganizations(): void {
    this.orgs
      ...
      .subscribe({
        next: (page) => {
          this.orgRows = page.rows.map((row) => ({
            id: row.id,
            name: row.applicant || row.name,
            classificationLabel: row.classificationLabel,
            overrideStages: row.kycRequiredApprovalStages ?? null,
            effectiveStages: row.effectiveKycRequiredApprovalStages ?? row.kycRequiredApprovalStages ?? this.policy.defaultRequiredApprovalStages,
            saving: false,
          }));
          this.cdr.markForCheck();
        },
