    ...
    this.usersAdmin
      ...
      .pipe(
        ...
        switchMap((prev) => {
          ...
          const save$ =
            this.securityId > 0
              ? this.usersAdmin.updateUserSecurity({ id: this.securityId, ...securityPayload })
              : this.usersAdmin.createUserSecurity(securityPayload);
          return save$;
        }),
        switchMap((secResp) => {
          if (this.usersAdmin.isUserMutationFailure(secResp)) {
