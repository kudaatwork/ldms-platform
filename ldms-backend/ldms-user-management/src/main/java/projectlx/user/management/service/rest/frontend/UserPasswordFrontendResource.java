...
@CrossOrigin
@RestController
@RequestMapping("/ldms-user-management/v1/frontend/user-password")
@Tag(name = "User Password Frontend Resource", description = "Operations related to managing user passwords")
@SecurityRequirement(name = "bearerAuth")
@RequiredArgsConstructor
public class UserPasswordFrontendResource {

    private final UserPasswordServiceProcessor userPasswordServiceProcessor;

    @Auditable(action = "CHANGE_USER_PASSWORD")
    @PreAuthorize("hasRole(T(projectlx.user.management.utils.security.UserPasswordRoles)." +
            "CHANGE_USER_PASSWORD.toString())")
    ...
    public UserPasswordResponse update(@Valid @RequestBody final ChangeUserPasswordRequest changeUserPasswordRequest,
        ...
        return userPasswordServiceProcessor.update(changeUserPasswordRequest, username, locale);
    }

    @Auditable(action = "RESET_PASSWORD")
    @PreAuthorize("hasRole(T(projectlx.user.management.utils.security.UserPasswordRoles)." +
            "RESET_USER_PASSWORD.toString())")
