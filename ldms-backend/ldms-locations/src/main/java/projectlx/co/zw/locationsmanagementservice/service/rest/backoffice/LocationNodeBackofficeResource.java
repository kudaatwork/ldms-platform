...
public class LocationNodeBackofficeResource {
    ...
    public LocationNodeResponse findAllAsAList(
            @Parameter(description = Constants.LOCALE_LANGUAGE_NARRATIVE)
            @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE) Locale locale) {
        return locationNodeServiceProcessor.findAllAsList(locale, "BACKOFFICE");
    }
    ...
    public ResponseEntity<byte[]> exportLocationNodes(@RequestBody LocationNodeMultipleFiltersRequest filters,
        ...
        try {
            ...
            LocationNodeResponse response = locationNodeServiceProcessor.findByMultipleFilters(filters, locale, "BACKOFFICE");
