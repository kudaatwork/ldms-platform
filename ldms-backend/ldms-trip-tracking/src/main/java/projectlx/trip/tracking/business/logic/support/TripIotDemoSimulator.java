...
public class TripIotDemoSimulator {
    ...
    private void advance(TripRoutePlan plan) {
        Trip trip = tripRepository.findByIdAndEntityStatusNot(plan.getTripId(), EntityStatus.DELETED).orElse(null);
        ...
        BigDecimal speed = plan.getCurrentSpeedKmh() != null
                ? plan.getCurrentSpeedKmh()
                : iotProperties.getSimulationSpeedKmh().asBigDecimal();
        ...
        recordCheckpoint(trip, lat, lng);
        TripLiveSnapshotDto snapshot = telemetryPublisher.buildSnapshot(
                trip, lat, lng, speed, plan.getCurrentHeadingDeg(), overall, true, true);
    ...
    private void recordCheckpoint(Trip trip, BigDecimal lat, BigDecimal lng) {
        ...
        event.setNotes("IoT GPS ping");
        event.setEntityStatus(EntityStatus.ACTIVE);
        event.setCreatedAt(now);
        event.setCreatedBy(SYSTEM_USER);
        tripEventServiceAuditable.create(event, java.util.Locale.ENGLISH, SYSTEM_USER);
    }
