...
@CrossOrigin
@RestController
@RequestMapping("/ldms-inventory-management/v1/system/purchase-order-line")
@Tag(name = "Purchase Order Line System Resource", description = "Operations related to managing purchase order lines (system)")
@RequiredArgsConstructor
public class PurchaseOrderLineSystemResource {

    private final PurchaseOrderLineServiceProcessor purchaseOrderLineServiceProcessor;
    private static final Logger logger = LoggerFactory.getLogger(PurchaseOrderLineSystemResource.class);
    ...
    public ResponseEntity<PurchaseOrderLineResponse> create(@Valid @RequestBody final CreatePurchaseOrderLineRequest request,
        ...
        return ResponseEntity.ok(purchaseOrderLineServiceProcessor.create(request, locale, "SYSTEM"));
    }
    ...
    public ResponseEntity<PurchaseOrderLineResponse> update(@Valid @RequestBody final EditPurchaseOrderLineRequest request,
                                 ...
                                 final Locale locale) {
        return ResponseEntity.ok(purchaseOrderLineServiceProcessor.update(request, "SYSTEM", locale));
    }
    ...
    public ResponseEntity<PurchaseOrderLineResponse> findById(@PathVariable("id") final Long id,
                                   ...
                                   @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                   final Locale locale) {
        return ResponseEntity.ok(purchaseOrderLineServiceProcessor.findById(id, locale, "SYSTEM"));
    }
    ...
    public ResponseEntity<PurchaseOrderLineResponse> delete(@PathVariable("id") final Long id,
                                 ...
                                 @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                 final Locale locale) {
        return ResponseEntity.ok(purchaseOrderLineServiceProcessor.delete(id, locale, "SYSTEM"));
    }
    ...
    public ResponseEntity<PurchaseOrderLineResponse> findAllAsAList(@Parameter(description = Constants.LOCALE_LANGUAGE_NARRATIVE)
                                         @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                             final Locale locale) {
        return ResponseEntity.ok(purchaseOrderLineServiceProcessor.findAllAsList(locale, "SYSTEM"));
    }
    ...
    public ResponseEntity<PurchaseOrderLineResponse> findByMultipleFilters(@Valid @RequestBody PurchaseOrderLineMultipleFiltersRequest filters,
                                               ...
                                               @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                               final Locale locale) {
        return ResponseEntity.ok(purchaseOrderLineServiceProcessor.findByMultipleFilters(filters, "SYSTEM", locale));
    }
    ...
    public ResponseEntity<byte[]> export(@RequestBody PurchaseOrderLineMultipleFiltersRequest filters,
        ...
        try {
            ...
            PurchaseOrderLineResponse response = purchaseOrderLineServiceProcessor.findByMultipleFilters(filters, "SYSTEM", locale);
    ...
    public ResponseEntity<ImportSummary> importFromCsv(@RequestParam("file") MultipartFile file) {
        try {
            logger.info("Incoming request to import purchase order lines from CSV file: {}", file.getOriginalFilename());
            if (file.isEmpty()) {
