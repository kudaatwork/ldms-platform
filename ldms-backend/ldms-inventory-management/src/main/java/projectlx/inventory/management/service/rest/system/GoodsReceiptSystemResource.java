...
@CrossOrigin
@RestController
@RequestMapping("/ldms-inventory-management/v1/system/purchase-orders")
@Tag(name = "Goods Receipt System Resource", description = "Operations related to receiving goods against purchase orders (system)")
@RequiredArgsConstructor
public class GoodsReceiptSystemResource {

    private final GoodsReceiptServiceProcessor goodsReceiptServiceProcessor;
    private static final Logger logger = LoggerFactory.getLogger(GoodsReceiptSystemResource.class);
    ...
    public PurchaseOrderResponse receive(@PathVariable("purchaseOrderId") final Long purchaseOrderId,
                                         ...
                                         final Locale locale) {
        logger.info("Incoming request to receive goods for PO {}", purchaseOrderId);
        request.setPurchaseOrderId(purchaseOrderId);
        return goodsReceiptServiceProcessor.receiveGoods(request, locale, "SYSTEM");
    }
}
