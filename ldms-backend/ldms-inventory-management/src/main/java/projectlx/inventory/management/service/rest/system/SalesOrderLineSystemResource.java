...
@CrossOrigin
@RestController
@RequestMapping("/ldms-inventory-management/v1/system/sales-order-line")
@Tag(name = "Sales Order Line System Resource", description = "Operations related to managing sales order lines (system)")
@RequiredArgsConstructor
public class SalesOrderLineSystemResource {

    private final SalesOrderLineServiceProcessor salesOrderLineServiceProcessor;
    private static final Logger logger = LoggerFactory.getLogger(SalesOrderLineSystemResource.class);
    ...
    public ResponseEntity<SalesOrderLineResponse> create(@Valid @RequestBody final CreateSalesOrderLineRequest request,
        ...
        return ResponseEntity.ok(salesOrderLineServiceProcessor.create(request, locale, "SYSTEM"));
    }
    ...
    public ResponseEntity<SalesOrderLineResponse> update(@Valid @RequestBody final EditSalesOrderLineRequest request,
                                 ...
                                 final Locale locale) {
        return ResponseEntity.ok(salesOrderLineServiceProcessor.update(request, "SYSTEM", locale));
    }
    ...
    public ResponseEntity<SalesOrderLineResponse> findById(@PathVariable("id") final Long id,
                                   ...
                                   @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                   final Locale locale) {
        return ResponseEntity.ok(salesOrderLineServiceProcessor.findById(id, locale, "SYSTEM"));
    }
    ...
    public ResponseEntity<SalesOrderLineResponse> delete(@PathVariable("id") final Long id,
                                 ...
                                 @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                 final Locale locale) {
        return ResponseEntity.ok(salesOrderLineServiceProcessor.delete(id, locale, "SYSTEM"));
    }
    ...
    public ResponseEntity<SalesOrderLineResponse> findAllAsAList(@Parameter(description = Constants.LOCALE_LANGUAGE_NARRATIVE)
                                         @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                             final Locale locale) {
        return ResponseEntity.ok(salesOrderLineServiceProcessor.findAllAsList(locale, "SYSTEM"));
    }
    ...
    public ResponseEntity<SalesOrderLineResponse> findByMultipleFilters(@Valid @RequestBody SalesOrderLineMultipleFiltersRequest filters,
                                               ...
                                               @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                               final Locale locale) {
        return ResponseEntity.ok(salesOrderLineServiceProcessor.findByMultipleFilters(filters, "SYSTEM", locale));
    }
    ...
    public ResponseEntity<byte[]> export(@RequestBody SalesOrderLineMultipleFiltersRequest filters,
        ...
        try {
            ...
            SalesOrderLineResponse response = salesOrderLineServiceProcessor.findByMultipleFilters(filters, "SYSTEM", locale);
    ...
    public ResponseEntity<ImportSummary> importFromCsv(@RequestParam("file") MultipartFile file) {
        try {
            logger.info("Incoming request to import sales order lines from CSV file: {}", file.getOriginalFilename());
            if (file.isEmpty()) {
