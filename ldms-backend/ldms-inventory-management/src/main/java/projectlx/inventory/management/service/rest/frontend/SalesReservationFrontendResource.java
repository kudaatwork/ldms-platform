...
@CrossOrigin
@RestController
@RequestMapping("/ldms-inventory-management/v1/frontend/sales-reservation")
@Tag(name = "Sales Reservation Frontend Resource", description = "Operations related to managing sales reservations")
@RequiredArgsConstructor
public class SalesReservationFrontendResource {

    private final SalesReservationServiceProcessor salesReservationServiceProcessor;
    private static final Logger logger = LoggerFactory.getLogger(SalesReservationFrontendResource.class);
    ...
    public SalesReservationResponse create(@Valid @RequestBody final CreateSalesReservationRequest request,
        ...
        return salesReservationServiceProcessor.create(request, locale, username);
    }
    ...
    public SalesReservationResponse update(@Valid @RequestBody final EditSalesReservationRequest request,
                                 ...
                                 final Locale locale) {
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        return salesReservationServiceProcessor.update(request, username, locale);
    }
    ...
    public SalesReservationResponse findById(@PathVariable("id") final Long id,
                                   ...
                                   @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE) final Locale locale) {
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        return salesReservationServiceProcessor.findById(id, locale, username);
    }
    ...
    public SalesReservationResponse delete(@PathVariable("id") final Long id,
                                 ...
                                 @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE) final Locale locale) {
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        return salesReservationServiceProcessor.delete(id, locale, username);
    }
    ...
    public SalesReservationResponse findAllAsAList(@Parameter(description = Constants.LOCALE_LANGUAGE_NARRATIVE)
                                         @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE) final Locale locale) {
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        return salesReservationServiceProcessor.findAllAsList(locale, username);
    }
    ...
    public SalesReservationResponse findByMultipleFilters(@Valid @RequestBody SalesReservationMultipleFiltersRequest filters,
                                               ...
                                               @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE) final Locale locale) {
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        return salesReservationServiceProcessor.findByMultipleFilters(filters, username, locale);
    }
    ...
    public ResponseEntity<byte[]> export(@RequestBody SalesReservationMultipleFiltersRequest filters,
        ...
        try {
            ...
            SalesReservationResponse response = salesReservationServiceProcessor.findByMultipleFilters(filters, username, locale);
    ...
    public ResponseEntity<ImportSummary> importFromCsv(@RequestParam("file") MultipartFile file) {
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        try {
            logger.info("Incoming request by '{}' to import sales reservations from CSV file: {}", username, file.getOriginalFilename());
            if (file.isEmpty()) {
