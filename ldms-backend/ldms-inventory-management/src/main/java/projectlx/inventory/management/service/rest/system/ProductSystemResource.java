...
@CrossOrigin
@RestController
@RequestMapping("/ldms-inventory-management/v1/system/product")
@Tag(name = "Product System Resource", description = "Operations related to managing products (system)")
@RequiredArgsConstructor
public class ProductSystemResource {

    private final ProductServiceProcessor productServiceProcessor;
    private static final Logger logger = LoggerFactory.getLogger(ProductSystemResource.class);
    ...
    public ResponseEntity<ProductResponse> create(@Valid @ModelAttribute final CreateProductRequest request,
                                @Parameter(description = Constants.LOCALE_LANGUAGE_NARRATIVE)
                                @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE) final Locale locale) {
        return ResponseEntity.ok(productServiceProcessor.create(request, locale, "SYSTEM"));
    }
    ...
    public ResponseEntity<ProductResponse> update(@Valid @ModelAttribute final EditProductRequest request,
                                 @Parameter(description = Constants.LOCALE_LANGUAGE_NARRATIVE)
                                 @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                 final Locale locale) {
        return ResponseEntity.ok(productServiceProcessor.update(request, "SYSTEM", locale));
    }
    ...
    public ResponseEntity<ProductResponse> findById(@PathVariable("id") final Long id,
                                   ...
                                   @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                   final Locale locale) {
        return ResponseEntity.ok(productServiceProcessor.findById(id, locale, "SYSTEM"));
    }
    ...
    public ResponseEntity<ProductResponse> delete(@PathVariable("id") final Long id,
                                 ...
                                 @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                 final Locale locale) {
        return ResponseEntity.ok(productServiceProcessor.delete(id, locale, "SYSTEM"));
    }
    ...
    public ResponseEntity<ProductResponse> findAllAsAList(@Parameter(description = Constants.LOCALE_LANGUAGE_NARRATIVE)
                                         @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                             final Locale locale) {
        return ResponseEntity.ok(productServiceProcessor.findAllAsList(locale, "SYSTEM"));
    }
    ...
    public ResponseEntity<ProductResponse> findByMultipleFilters(@Valid @RequestBody ProductMultipleFiltersRequest filters,
                                               ...
                                               @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                               final Locale locale) {
        return ResponseEntity.ok(productServiceProcessor.findByMultipleFilters(filters, "SYSTEM", locale));
    }
    ...
    public ResponseEntity<byte[]> export(@RequestBody ProductMultipleFiltersRequest filters,
        ...
        try {
            ...
            ProductResponse response = productServiceProcessor.findByMultipleFilters(filters, "SYSTEM", locale);
