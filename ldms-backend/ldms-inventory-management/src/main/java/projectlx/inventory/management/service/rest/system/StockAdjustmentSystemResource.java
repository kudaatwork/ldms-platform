...
@CrossOrigin
@RestController
@RequestMapping("/ldms-inventory-management/v1/system/stock-adjustment")
@Tag(name = "Stock Adjustment System Resource", description = "Operations related to managing stock adjustments (system)")
@RequiredArgsConstructor
public class StockAdjustmentSystemResource {

    private final StockAdjustmentServiceProcessor stockAdjustmentServiceProcessor;
    private static final Logger logger = LoggerFactory.getLogger(StockAdjustmentSystemResource.class);
    ...
    public StockAdjustmentResponse create(@Valid @RequestBody final CreateStockAdjustmentRequest request,
                                @Parameter(description = Constants.LOCALE_LANGUAGE_NARRATIVE)
                                @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE) final Locale locale) {
        return stockAdjustmentServiceProcessor.create(request, locale, "SYSTEM");
    }
    ...
    public StockAdjustmentResponse update(@Valid @RequestBody final EditStockAdjustmentRequest request,
                                 @Parameter(description = Constants.LOCALE_LANGUAGE_NARRATIVE)
                                 @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                 final Locale locale) {
        return stockAdjustmentServiceProcessor.update(request, "SYSTEM", locale);
    }
    ...
    public StockAdjustmentResponse findById(@PathVariable("id") final Long id,
                                   @Parameter(description = Constants.LOCALE_LANGUAGE_NARRATIVE)
                                   @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                   final Locale locale) {
        return stockAdjustmentServiceProcessor.findById(id, locale, "SYSTEM");
    }

    @Auditable(action = "DELETE_STOCK_ADJUSTMENT")
    @DeleteMapping(value = "/delete-by-id/{id}")
    @Operation(summary = "Delete a stock adjustment by ID")
    public StockAdjustmentResponse delete(@PathVariable("id") final Long id,
                                 @Parameter(description = Constants.LOCALE_LANGUAGE_NARRATIVE)
                                 @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                 final Locale locale) {
        return stockAdjustmentServiceProcessor.delete(id, locale, "SYSTEM");
    }
    ...
    public StockAdjustmentResponse findAllAsAList(@Parameter(description = Constants.LOCALE_LANGUAGE_NARRATIVE)
                                         @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                             final Locale locale) {
        return stockAdjustmentServiceProcessor.findAllAsList(locale, "SYSTEM");
    }
    ...
    public StockAdjustmentResponse findByMultipleFilters(@Valid @RequestBody StockAdjustmentMultipleFiltersRequest filters,
                                               @Parameter(description = Constants.LOCALE_LANGUAGE_NARRATIVE)
                                               @RequestHeader(value = Constants.LOCALE_LANGUAGE, defaultValue = Constants.DEFAULT_LOCALE)
                                               final Locale locale) {
        return stockAdjustmentServiceProcessor.findByMultipleFilters(filters, "SYSTEM", locale);
    }
    ...
    public ResponseEntity<byte[]> export(@RequestBody StockAdjustmentMultipleFiltersRequest filters,
        ...
        try {
            ...
            StockAdjustmentResponse response = stockAdjustmentServiceProcessor.findByMultipleFilters(filters, "SYSTEM", locale);
    ...
    public ResponseEntity<ImportSummary> importFromCsv(@RequestParam("file") MultipartFile file) {
        try {
            logger.info("Incoming request to import stock adjustments from CSV file: {}", file.getOriginalFilename());
            if (file.isEmpty()) {
