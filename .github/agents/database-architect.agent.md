...
2. **Always use utf8mb4** charset with unicode_ci collation
3. **Always include audit fields** on every table
4. **Use BIGINT for all IDs**
5. **Use VARCHAR for enums** (never MySQL ENUM type)
6. **Use DECIMAL for money/quantities** with appropriate precision
7. **Use DATETIME(6)** for timestamp precision
8. **Index foreign keys** and frequently queried columns
9. **Use ON DELETE RESTRICT** for foreign keys
10. **Soft deletes** via entity_status, never physical DELETE
